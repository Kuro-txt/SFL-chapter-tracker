...
export async function saveProgressToCloudKV(silent = false) {
  ...
  try {
    ...
    if (!res.ok || responseData.error) throw new Error(responseData.error || 'Failed to save.');

    state.currentVaultData = responseData.vaultData;
    if (state.globalData) {
      state.globalData.cloudHistory = responseData.vaultData;
    }

    recalculateAll();

    if (!silent) {
      const totalTix = responseData.vaultData?.cumulativeTickets || 0;
