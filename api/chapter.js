...
async function handleRequest(request) {
  ...
  // 1. Vault Fetch Endpoint
  if (action === 'getVault') {
    const username = (url.searchParams.get('username') || '').toLowerCase().trim();
    if (!username) return jsonRes({ vaultData: null });

    const client = await pool.connect();
    try {
      const res = await client.query('SELECT vault_data FROM user_vaults WHERE username = $1', [username]);
  ...
  if (request.method === 'POST' && action === 'login') {
    ...
    const client = await pool.connect();
    try {
      const res = await client.query('SELECT auth_data, vault_data FROM user_vaults WHERE username = $1', [username]);
